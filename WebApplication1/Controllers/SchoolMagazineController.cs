...
namespace WebApplication1.Controllers
{
    public class SchoolMagazineController : Controller
    {
        ...
        public IActionResult EditMagazineAndView(int Class, string ClassLetter, List<string> daysOfSubjects, DateTime Start, DateTime End, string Q)
        {
            User user = db.Users.Where(i => i.Id == getCurrentUserId()).First();
            ...
            foreach (var m in mag)
            {
                model.Date.Add(Convert.ToDateTime(m.Date));
                model.Pupils_marks.Add("f");
            }
            return View(model);
        }
