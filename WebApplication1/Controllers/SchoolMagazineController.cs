    ...
    public class SchoolMagazineController : Controller
    {
        ...
        public IActionResult SchoolMagazine(int Class, string ClassLetter, List<string> daysOfSubjects, DateTime Start, DateTime End, string Q)
        {
            ...
            var subjects = (from t in db.Timetable
                            ...
                            && c.ClassNumber == Class && c.ClassLetter == ClassLetter && e.RegistrateSchoolId == user.SchoolId
        ...
        public async Task<IActionResult> ViewMagazine(string Subject, int Class, string ClassLetter, string Quarters)
        {
            ...
            var check = (from sm in db.SchoolMagazine
                         join ac in db.AcademicPerformance on sm.Id equals ac.SchoolMagazineId
                         join sc in db.SchoolClasses on sm.SchoolClassesId equals sc.Id
                         join c in db.Class on sc.ClassId equals c.Id
                         join s in db.Subject on sm.SubjectId equals s.Id
                         where c.ClassLetter == ClassLetter && c.ClassNumber == Class && s.Name == Subject && sc.SchoolId == user.SchoolId
                         select sm).Count();
            if (check == 0)
            {
                return RedirectToAction("SchoolMagazine", new { Class = Class, ClassLetter = ClassLetter});
            }
            else
            {
        ...
        public async Task<IActionResult> EditMagazine(SchoolMagazineViewModel model, int Class, string ClassLetter, string Quarters, string Subject)
        {
            User user = db.Users.Where(i => i.Id == getCurrentUserId()).First();

            var magaz = (from sm in db.SchoolMagazine
                         join ac in db.AcademicPerformance on sm.Id equals ac.SchoolMagazineId
                         join sc in db.SchoolClasses on sm.SchoolClassesId equals sc.Id
                         join c in db.Class on sc.ClassId equals c.Id
                         join s in db.Subject on sm.SubjectId equals s.Id
                         where c.ClassLetter == ClassLetter && c.ClassNumber == Class && s.Name == Subject && sc.SchoolId == user.SchoolId
                         select sm).ToList();
            var academ = (from sm in db.SchoolMagazine
                          join ac in db.AcademicPerformance on sm.Id equals ac.SchoolMagazineId
                          join sc in db.SchoolClasses on sm.SchoolClassesId equals sc.Id
                          join c in db.Class on sc.ClassId equals c.Id
                          join s in db.Subject on sm.SubjectId equals s.Id
                          where c.ClassLetter == ClassLetter && c.ClassNumber == Class && s.Name == Subject && sc.SchoolId == user.SchoolId
                          select ac).ToList();
            db.SchoolMagazine.RemoveRange(magaz);
            db.AcademicPerformance.RemoveRange(academ);
            await db.SaveChangesAsync();

            Subject subject = db.Subject.Where(s => s.Name == Subject).First();
            Employee emp = db.Employee.Where(e => e.UserId == getCurrentUserId()).First();
            var sch = (from s in db.SchoolClasses
                       join c in db.Class on s.ClassId equals c.Id
                       where s.SchoolId == emp.RegistrateSchoolId && c.ClassNumber == Class && c.ClassLetter == ClassLetter
            ...
            for (int i = 0; i < pupils.Count; i++)
            {
                for (int j = 0; j < model.Pupils_marks[i].Count; j += dates.Count + 1)
                {
                    pupils_marks.Add(model.Pupils_marks[i].GetRange(j, dates.Count + 1));
                }
            }
